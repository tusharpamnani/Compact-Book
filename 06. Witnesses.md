...
> **Examples:** [06.01 Witnesses](../examples/06.01.witnesses.compact)
...
│                                                             │
│  const witnesses = {                                        │
│    callerAddress: (ctx) => [null, deriveAddress(wallet)]  │
│  };                                                         │
│  const contract = new Contract(witnesses);                  │
│                                                             │
│  await contract.circuits.mint(ctx, metadataHash);          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
                           │
                           ▼
...
│                                                             │
│  witness secretKey(): Bytes<32>;  // declared in Compact    │
│                                                             │
│  export circuit mint(...): [] {                            │
│    const sk = secretKey();  // called at runtime          │
│    // sk is now available to the circuit                   │
│    // sk NEVER goes on-chain                               │
│  }                                                          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
                           │
...
│                                                             │
│  Proof server generates:                                    │
│  - "mint executed correctly"                               │
│  - "caller is the owner"                                   │
│  - NO secrets revealed                                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│  Validators verify:                                         │
│  - The proof is valid                                       │
│  - The state update is authorized                          │
│  - They never saw the secret key                           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
- **Previous:** [Circuits](./05.%20Circuits.md)  Constraint declarations
- **Next:** [Explicit Disclosure](./07.%20Explicit%20Disclosure.md)  Deep dive on disclose()
- **See also:** [Ledger State](./04.%20Ledger%20State.md)  Public vs private
- **See also:** [Writing a Contract](./03.%20Writing%20A%20Contract.md)  Full example
- **Examples:** [06.01 Witnesses](../examples/06.01.witnesses.compact)